...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        User = get_user_model()
        self.stdout.write(self.style.SUCCESS("--- Iniciando sincronização Conta Azul (Busca Dupla) ---"))
        ...
        for user in User.objects.filter(is_active=True):
            ...
            # --- Bloco de Atualização de Token (Mantido igual) ---
            access_token = token_obj.access_token
            headers = {}
            now = timezone.now()
            if not token_obj.expires_at or token_obj.expires_at <= (now + timedelta(minutes=5)):
            ...
            data_inicio_str = data_inicio_dt.strftime('%Y-%m-%d')
            data_fim_str = data_fim_dt.strftime('%Y-%m-%d')
            # Data futura para pegar vencimentos próximos (30 dias)
            data_futura_str = (data_fim_dt + timedelta(days=30)).strftime('%Y-%m-%d')

            # =========================================================================
            # SINCRONIZAÇÃO DE CONTAS A RECEBER (BUSCA DUPLA)
            # =========================================================================
            
            # Definimos as duas estratégias de busca:
            # 1. Pelo VENCIMENTO: Pega o que está em aberto ou atrasado (Janela: Passado recente + Futuro)
            # 2. Pelo PAGAMENTO: Pega o que foi baixado recentemente (Janela: Passado recente)
            estrategias_receber = [
                {
                    'nome': 'Por Vencimento',
                    'params': {'status': ['EM_ABERTO', 'ATRASADO'], 'data_vencimento_de': data_inicio_str, 'data_vencimento_ate': data_futura_str}
                },
                {
                    'nome': 'Por Pagamento',
                    'params': {'status': ['RECEBIDO'], 'data_pagamento_de': data_inicio_str, 'data_pagamento_ate': data_fim_str}
                }
            ]
            
            self.stdout.write("--- Buscando Contas a Receber (Estratégia Dupla: Vencimento & Pagamento) ---")
            
            # Loop externo para rodar as duas estratégias
            for estrategia in estrategias_receber:
                self.stdout.write(f"  > Iniciando busca C.Receber: {estrategia['nome']}...")
                page = 1
                size = 50
                ...
                while True:
                    ...
                    # Monta os parâmetros baseados na estratégia atual
                    params_receber = {
                        'pagina': page,
                        'tamanho_pagina': size
                    }
                    params_receber.update(estrategia['params'])

                    self.stdout.write(f"    Buscando Pág {page} ({estrategia['nome']})...")
                    response_receber = call_api_with_retry(url_receber, params=params_receber)

                    if response_receber is None or response_receber.status_code != 200:
                        self.stderr.write(self.style.ERROR(f"Erro crítico ao buscar C.Receber ({estrategia['nome']})."))
                        break
                    try:
                        response_data = response_receber.json()
                        contas_receber_ca = response_data.get('itens', [])
                        if not contas_receber_ca:
                            self.stdout.write(f"    Nenhuma C.Receber encontrada nesta página ({estrategia['nome']}). Próxima...")
                            break

                        self.stdout.write(f"    Processando {len(contas_receber_ca)} C.Receber...")
                        
                        # --- AQUI ESTÁ O SEU CÓDIGO ORIGINAL DE PROCESSAMENTO (MANTIDO INTACTO) ---
                        for conta in contas_receber_ca:
                            try:
                                status_ca = conta.get('status_traduzido')
                                is_received_ca = status_ca == 'RECEBIDO'
                                ...
                                # 1. Verifica se já existe (BLINDAGEM)
                                try:
                                    obj_existente = ReceivableAccount.objects.get(external_id=conta.get('id'))
                                    existe = True
                                except ReceivableAccount.DoesNotExist:
                                ...
                                # 5. Refinamento Inteligente
                                if not created and obj.category.name == 'Receitas de Vendas' and cat_inteligente:
                                     obj.category = cat_inteligente
                                     obj.dre_area = dre_inteligente if dre_inteligente else obj.dre_area
                                     if banco_inteligente: obj.bank_account = banco_inteligente
                                     obj.save()
                                     # self.stdout.write(self.style.SUCCESS(f"      -> [Smart Update] Categoria atualizada."))

                            except Exception as e:
                                self.stderr.write(self.style.ERROR(f"Erro C.Receber ID {conta.get('id', 'N/A')}: {e}"))
                                # print(traceback.format_exc()) # Opcional: comentar para limpar log
                        # --- FIM DO BLOCO ORIGINAL DE PROCESSAMENTO ---

                    except Exception as e:
                         self.stderr.write(self.style.ERROR(f"Erro geral no loop C.Receber: {e}"))
                         break
                    page += 1
                    time.sleep(1)

            # =========================================================================
            # SINCRONIZAÇÃO DE CONTAS A PAGAR (BUSCA DUPLA)
            # =========================================================================
            ...
            self.stdout.write("--- Buscando Contas a Pagar (Estratégia Dupla: Vencimento & Pagamento) ---")

            for estrategia in estrategias_pagar:
                self.stdout.write(f"  > Iniciando busca C.Pagar: {estrategia['nome']}...")
                page = 1
                ...
                while True:
                    ...
                    params_pagar.update(estrategia['params'])

                    self.stdout.write(f"    Buscando Pág {page} ({estrategia['nome']})...")
                    response_pagar = call_api_with_retry(url_pagar, params=params_pagar)

                    if response_pagar is None or response_pagar.status_code != 200:
                        self.stderr.write(self.style.ERROR(f"Erro crítico ao buscar C.Pagar ({estrategia['nome']})."))
                        break
                    try:
                        response_data = response_pagar.json()
                        contas_pagar_ca = response_data.get('itens', [])
                        if not contas_pagar_ca:
                            self.stdout.write(f"    Nenhuma C.Pagar encontrada nesta página ({estrategia['nome']}). Próxima...")
                            break

                        self.stdout.write(f"    Processando {len(contas_pagar_ca)} C.Pagar...")
                        ...
                        for conta in contas_pagar_ca:
                            try:
                                ...
                                self.stderr.write(self.style.ERROR(f"Erro C.Pagar ID {conta.get('id', 'N/A')}: {e}"))
                                # print(traceback.format_exc())

                        # --- FIM DO BLOCO ORIGINAL DE PROCESSAMENTO ---

                    except Exception as e:
                         self.stderr.write(self.style.ERROR(f"Erro geral no loop C.Pagar: {e}"))
                         break
