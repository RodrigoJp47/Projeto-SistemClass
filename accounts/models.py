...
# No topo do arquivo, garanta que tem esses imports:
from django.db import models
from django.contrib.auth.models import User
import os

# Função para definir onde salvar os certificados (pasta separada por user)
def upload_cert_path(instance, filename):
    return f'certs/user_{instance.user.id}/{filename}'

class InterCredentials(models.Model):
    user = models.OneToOneField(User, on_delete=models.CASCADE, related_name='inter_creds')
    ...
    # Arquivos
    certificado_crt = models.FileField(upload_to=upload_cert_path, verbose_name="Arquivo .crt")
    chave_key = models.FileField(upload_to=upload_cert_path, verbose_name="Arquivo .key")
