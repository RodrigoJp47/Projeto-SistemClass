...
from django.contrib.auth.decorators import login_required
from accounts.decorators import subscription_required, module_access_required, check_employee_permission
...
@module_access_required('commercial') # Garante que a empresa paga pelo módulo comercial
@check_employee_permission('can_access_crm') # Garante que o funcionário tem a permissão específica
def crm_kanban_view(request):
    """
    View principal do Kanban: Gerencia a exibição, criação de oportunidades e garante as etapas padrão.
    """
    ...
    etapas_obrigatorias = [
        ...
        {'nome': 'Negociação', 'cor': '#9b59b6', 'ordem': 3},
        {'nome': 'Fechamento', 'cor': '#2ecc71', 'ordem': 4},
        {'nome': 'Negado', 'cor': '#e74c3c', 'ordem': 5}
    ]
...
@login_required
@subscription_required
@check_employee_permission('can_access_crm')
def mover_oportunidade_ajax(request):
    if request.method == 'POST':
        try:
            data = json.loads(request.body)
